...
public class BulletSpawnerOnAlly : MonoBehaviour {
    ...
    /// <summary>
    /// return a v3 with a random position in a circle around the gameObject
    /// </summary>
    /// <returns></returns>
    ...
    IEnumerator Spawn(GameObject bullet, int damage, Vector3 position, Vector3 targetPosition)
    {
        ...
        instBullet.GetComponent<CannonBullet>().targetID = 0;
        instBullet.GetComponent<CannonBullet>().bulletDamage = damage;
        instBullet.GetComponent<ShaderRenderOrder>().renderQueueValue = 2000;
    }
