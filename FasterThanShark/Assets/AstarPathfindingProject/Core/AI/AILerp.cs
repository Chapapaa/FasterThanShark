...
public class AILerp : MonoBehaviour {
	...
	public float TrySearchPath () {
		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && target != Vector3.zero) {
			SearchPath();
			return repathRate;
		} else {
	...
	public virtual void ForceSearchPath () {
		if (target == Vector3.zero) throw new System.InvalidOperationException("Target is null");
